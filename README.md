...
Token checks have a 12-second overall deadline. If the website's RPC fails or takes more than 6 seconds, the check falls back to the connected wallet's provider, using read-only `eth_call` requests for the same balance, decimals and symbol. The provider's actual chain is checked before and after these reads. No signature or transaction is requested. A successful direct read, including a zero balance or mismatched metadata, never triggers this fallback. The HTTP transport retains an 8-second request timeout with automatic transport/query retries disabled. A failed check offers **Check again** and never treats a network error as a zero balance. Offline queries show **You're offline** and resume on reconnect. Successful reads continue refreshing every 30 seconds; failed checks stop periodic retries. The public Robinhood endpoint is rate-limited and is not recommended for production in the linked official documentation. Configure a production provider through `NEXT_PUBLIC_ROBINHOOD_RPC_URL` and rebuild/redeploy when changing that public setting. Wallet recovery also needs a working provider; it cannot restore access if both connections fail.
...
Use `--wallet-rpc --flow-only` to include recovery through a simulated wallet when the website RPC closes or stalls, plus zero-balance, metadata and actual-provider-network checks. This also runs the token-check scenarios. Browser tests use isolated wallet/RPC responses; they do not verify a live user's balance.
...
The homepage exposes `/dead-drop` through the Tools navigation and its own tool choice, independently of the hex converter's wallet gate. Dead drop has an invitation-code → message → share-link flow. Recipients use `/dead-drop/[id]#key` without a wallet. The complete link grants access to its encrypted message for 24 hours; opening it does not delete it. Open sender and recipient pages clear expired links, keys and displayed plaintext, including on focus/visibility restoration. Responses without a valid future expiry cannot reveal a message. The key cannot be recovered if the sender loses the link. This route does not establish deployment or Tor-service availability.

### Automatic message purge

`GET /api/cron/purge-drops` deletes expired ciphertext independently of message creation/retrieval when called by a scheduler. Configure a random server-only `CRON_SECRET` of at least 32 characters in production and send `Authorization: Bearer <CRON_SECRET>`. Never use a `NEXT_PUBLIC_` variable or a query-string secret. Missing configuration fails with 503; unauthorized requests fail with 401. HEAD never runs cleanup.

Each invocation deletes only records whose `expires_at <= NOW()` according to PostgreSQL, in up to twenty batches of 1,000. Row locks with `SKIP LOCKED` permit safe overlapping invocations. The response contains counts, never messages or keys. Unfinished backlog or database failure returns 503 and emits a generic failure log, so the scheduler can retry and alert. The job has a 45-second client deadline and a 60-second function limit. A later invocation safely continues unfinished work.

The schedule lives in `.github/workflows/purge-expired-drops.yml`. GitHub Actions calls the production endpoint every five minutes at minutes 2, 7, 12, and so on (UTC). It supports manual **Run workflow**, prevents overlapping workflow runs, and retries failed or incomplete cleanup up to three times. No repository checkout or third-party action is required, and the GitHub token has no permissions. Enable Actions failure notifications for the account maintaining this schedule.

To activate, set the same random `CRON_SECRET` in **Vercel → hex-dump → Production environment variables** and **GitHub → hex-dump → Settings → Secrets and variables → Actions → Repository secrets**. Deploy the purge endpoint with that setting and push the workflow to the default branch. Run **Purge expired messages** manually, then verify unattended runs in the Actions tab. Never paste the secret into the workflow or commit it. The workflow fails visibly if the secret is missing or the endpoint is unavailable.

This works with the current Vercel Hobby deployment because GitHub owns the schedule. [GitHub may delay or drop scheduled jobs and disables public-repository schedules after 60 days without repository activity](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule). Monitor runs and re-enable the workflow if it is disabled. Five minutes is the intended cadence, not a maximum deletion delay. No Vercel cron is needed.

Access is blocked at expiry even if cleanup is delayed. Physical deletion follows a successful scheduled run; provider outages, backlog, database restore history and recipient copies prevent a promise of immediate erasure from every storage layer. This feature adds no message history or recovery.

Run `node --experimental-strip-types tests/browser/dead-drop-expiry.mjs` against a local production server for open-tab expiry, resumed tabs, invalid expiry and delayed decryption. `tests/deadDropPurge.test.mjs` tests authorization, batches and failures. Its opt-in `DEAD_DROP_PURGE_TEST_DATABASE_URL` test executes the actual deletion SQL in transaction-scoped temporary tables, without deleting real messages.
