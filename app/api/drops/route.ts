...
import { validateDeadDropEnvelope, MAX_ENVELOPE_BYTES } from "@/lib/deadDropCrypto";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!(await verifyInvitation(tokenHash, idempotencyKey))) return NextResponse.json({ error: "Invitation unavailable." }, { status: 403, headers });
    ...
    if (encodedSize > MAX_ENVELOPE_BYTES) return NextResponse.json({ error: "Message is too large." }, { status: 413, headers });
