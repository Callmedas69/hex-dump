...
export function productionPolicy(env:Partial<NodeJS.ProcessEnv>={}): TokenPolicy | null {
  ...
  let minimum: bigint;
  try { minimum = BigInt(env.NEXT_PUBLIC_MEME_TOKEN_MIN_RAW ?? ""); } catch { return null; }
  if (!address || !/^[0-9a-f]{40}$/i.test(address.replace(/^0x/, "")) || !/^\d+$/.test(rawChainId ?? "") || !Number.isSafeInteger(chainId) || chainId !== 4663 || !symbol || !/^[A-Za-z0-9._-]{1,32}$/.test(symbol) || !/^\d+$/.test(rawDecimals ?? "") || !Number.isInteger(decimals) || decimals < 0 || decimals > 255 || minimum < 0n) return null;
  return { chainId, address: (address.startsWith("0x") ? address : `0x${address}`) as Address, symbol, decimals, minimumRawBalance: minimum };
}
