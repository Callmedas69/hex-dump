...
export function InvitationCreator() {
  ...
  const [deposits, setDeposits] = useState("10");
  ...
  async function create() {
    ...
    try {
      ...
      const response = await fetch("/api/invitations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ address, nonce: challenge.nonce, signature, deposits: Number(deposits), days: Number(days) }) });
  ...
  return <section className="invitation-creator" aria-labelledby="invitation-title"><div><span className="eyebrow">USDG HOLDER TOOL</span><h2 id="invitation-title">CREATE INVITATION</h2><p>Sign a one-time wallet challenge to create a limited deposit token. The invitation authorizes depositing; the complete dead-drop link authorizes retrieval.</p></div><div className="invitation-controls"><ConnectButton accountStatus="address" showBalance={false} /><label>Deposits<input inputMode="numeric" min="1" max="100" type="number" value={deposits} onChange={event => setDeposits(event.target.value)} /></label><label>Valid days<input inputMode="numeric" min="1" max="30" type="number" value={days} onChange={event => setDays(event.target.value)} /></label><button type="button" className="primary" disabled={!isConnected || busy} onClick={create}>{busy ? "Authorizing…" : "Create invitation"}</button></div>{token && <div className="invitation-token"><code>{token}</code><button type="button" onClick={() => void navigator.clipboard.writeText(token)}>Copy token</button></div>}<p className="notice" role="status" aria-live="polite">{notice}</p></section>;
}
