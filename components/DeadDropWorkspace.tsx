  ...
  return <main className="shell dead-drop-page" data-motion={motion ? "on" : "off"}>
    ...
    <section ref={workspace} className="workspace" aria-label="Secret dead drop">
      {expired ? <div className="dead-drop-expired"><h2 ref={resultHeading} tabIndex={-1}>MESSAGE EXPIRED</h2><p>This message has expired and has been cleared from this page.</p>{!reader && <button type="button" onClick={() => setExpired(false)}>Create another message</button>}</div> : reader ? <div className="dead-drop-reader">
        ...
        {created.onionLink ? <><label htmlFor="onion-message-link">1. Tor onion link</label><textarea id="onion-message-link" readOnly value={created.onionLink} rows={4} spellCheck={false} aria-describedby="message-link-help" onFocus={event => event.currentTarget.select()} /><div className="dead-drop-actions"><button type="button" className="primary" onClick={() => copyLink(created.onionLink, "Onion message link")}>Copy onion link</button><a className="dead-drop-open" href={created.onionLink}>Open onion link</a></div></> : <p className="notice">Tor onion link is not configured for this deployment.</p>}
        <label htmlFor="message-link">2. HTTPS link</label>
        ...
        <div className="dead-drop-actions"><button type="button" onClick={() => copyLink(created.link, "HTTPS message link")}>Copy message link</button><a className="dead-drop-open" href={created.link}>Open message</a></div>
